...
private fun getTypeRank2(hand: String) =
    hand.filter { it != 'J' }.groupingBy { it }.eachCount().values.sorted().let { counts ->
        counts.dropLast(1) + ((counts.lastOrNull() ?: 0) + hand.count { it == 'J' })
    }

private fun solution(lines: List<String>, getType: (String) -> Collection<Int>, cardOrder: String) = lines
    .map { line ->
    ...
    .sortedBy { it.first }
    .mapIndexed { idx, play -> (idx + 1) * play.second }
    .sum()
